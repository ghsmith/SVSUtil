...
The SVS files generated by our GT450 scanners have a TIFF image description that starts with "Aperio Leica Biosystems GT450 v1.0.1" and contain seven (7) TIFF directories in this order:
...
A positive feature of this design relative to the AT2, which you will read aboutin the next section, is that quality can be easily adjusted on a tile-by-tile basis since the tile bytes for each tile represent a completely atomic and renderable JPEG. This is harder to do when the JPEG quantization and Huffman tables are specified at the level of the TIFF directory, as they are with the AT2. A negative feature of this design relative to the AT2 is that there is a lot of redundant JPEG table data in the SVS since each tile has its own JPEG table data, although it only amounts to a couple of hundred bytes per tile.
...
The SVS files generated by our AT2 scanners have a TIFF image description that starts with "Aperio Image Library v12.0.15" and contain six (6) TIFF directories in this order:
...
- TIFF Directory #4 - Label. Horizontally-differenced LZW-encoded 24-bit color bitmap (not tiled). This directory does not include an ICC profile. This image is not recolored by "colorutil" but it is manipulated by "labelutil." Although this image is not tiled, it does use "classic" TIFF strips.
